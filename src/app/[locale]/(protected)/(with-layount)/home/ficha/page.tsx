...
export default function FichaPage() {
  const openPopupWindow = () => {
    const url = '/es/emerging/ficha' // Ruta o URL que deseas abrir
    const windowFeatures =
      'width=730,height=700,menubar=no,toolbar=no,status=no,scrollbars=no'

    const newWindow = window.open(url, '_blank', windowFeatures)
    if (newWindow) newWindow.focus() // Asegura que la ventana obtenga el foco
  }
