...
export default function Ficha({ emerging = false }: { emerging?: boolean }) {
  ...
  //////////////////////////////////////////
  // meter lockSheet, setLockSheet y emerging al estado flobal y al schema de prisma!!
  //////////////////////////////////////////

  const { data, isLoading } = useGetAllSheets()

  return (
    <>
      {data.response &&
        data.status === 'success' &&
        data.response.map((sheetData) => (
          <SheetStoreProvider sheetData={sheetData}>
            <article className='flex flex-col justify-start items-start bg-[#fffbdd] w-[700px] rounded-lg text-[#530800]'>
              {/* HEADER */}
              <MainHeader lockSheet={lockSheet} setLockSheet={setLockSheet} emerging={emerging} />
